...
export const reducerFetchMenu = (state = initalState, action) => {
  ...
  switch (action.type) {
    case actionTypes.START_FETCHING_MENU_BUSINESS_TWO:
      return {
        fetching: true
      }
    /** FETCH DATA FOR Music menu page */
    case actionTypes.FETCHED_MENU_BUSINESS_TWO:
      ...
      action.data.forEach(function (item) {
        arr.push({
          id: item.key,
          name: item.title,
          path: 'b4-business-two#' + item.uri.replace('base:', ''),
    ...
    case actionTypes.FETCH_ERROR_MENU_BUSINESS_TWO:
      return {
        fetched: false,
        fetching: false,
        error: action.error
