...
function SocialMediaSVG(props) {

  const [photo, setPhoto] = useState([]);
  const [indexId, setIndexId] = useState(0);
  const dataUrl = photoUrl.URL;
  const tagId = 20; //Taxonomy target_id for "Social Media SVG"
  ...
  console.log("Social Media", photo, 'Length', photo.length, 'index Id', indexId);
